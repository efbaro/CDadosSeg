...
def extract_manifest(path_apks, path_out):
    ...
    for file in files:
        ...
        os.system('mv \''+ file +'.out/AndroidManifest.xml\' '+ path_out + '/AndroidManifest_'+package_name+'_.xml' )
...
def salvar_csv(dic, nome_arquivo):
    """
    Salva dados do dicionário em um arquivo CSV
    """

    # Abre o arquivo para gravação
    arquivo_saida = open(nome_arquivo + '.csv', 'w')

    # Salva o cabeçalho
    arquivo_saida.write('NOME_APP,PERMISSOES\n')
    ...
    for chave, valores in dic.items():
        ...
        chave = chave.split('.apk')

        linha = chave[0] + ',' + str_valores + '\n'
...
def main():
    ...
    try:
        ...
        if len(dic) != 0:
            ...
            if args.file != None:
                salvar_csv(dic, args.file)
            else:
                salvar_csv(dic, 'arquivo')
